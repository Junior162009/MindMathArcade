const CACHE_NAME = 'banderquiz-v6';
...
function detectarContinenteDesdeZonaHoraria() {
    ...
    if (tz === 'America/Bogota' || tz === 'America/Lima' || tz === 'America/Guayaquil' || tz === 'America/La_Paz' || tz === 'America/Asuncion' || tz === 'America/Montevideo' || tz === 'America/Argentina/Buenos_Aires' || tz === 'America/Santiago' || tz === 'America/Caracas' || tz === 'America/Cayenne' || tz.startsWith('America/Sao_Paulo') || tz === 'America/Fortaleza' || tz === 'America/Recife' || tz === 'America/Belem' || tz === 'America/Manaus' || tz === 'America/Paramaribo') return 'south-america';
    if (tz.startsWith('Europe/')) return 'europe';
    if (tz.startsWith('Africa/')) return 'africa';
    if (tz.startsWith('Asia/')) return 'asia';
    if (tz.startsWith('Australia/') || tz.startsWith('Pacific/')) return 'oceania';
    if (tz === 'America/New_York' || tz === 'America/Chicago' || tz === 'America/Denver' || tz === 'America/Los_Angeles' || tz === 'America/Toronto' || tz === 'America/Vancouver' || tz === 'America/Anchorage') return 'north-america';
    ...
    const adaptiveFunction = `
        ...
        function bqSetFiltered() {
            ...
            if (currentLevel === 'easy') {
                ...
                filteredCountries = local.length >= 4 ? local : COUNTRIES.slice(0, 60);
            } else {
                filteredCountries = localSet ? COUNTRIES.filter(c => !localSet.has(c.cca2)) : COUNTRIES.slice();
            }
        }
...
(function () {
    ...
    const REGION_NAME = {'south-america':'América del Sur','central-america':'Centroamérica y Caribe','north-america':'América del Norte','europe':'Europa','africa':'África','asia':'Asia','oceania':'Oceanía','unknown':'tu región'};
    const continent = '${detectarContinenteDesdeZonaHoraria()}';

    function emojiAISO(emoji) {
        const chars = [...String(emoji || '')].map(c => c.codePointAt(0)).filter(cp => cp >= 0x1F1E6 && cp <= 0x1F1FF);
    ...
    function actualizarRegion() {
        ...
        if (invite && document.querySelector('.portal')) {
            const text = '📍 ' + (REGION_NAME[continent] || REGION_NAME.unknown) + ' · dificultad adaptada automáticamente';
            if (invite.textContent !== text) invite.textContent = text;
        }
        const cards = document.querySelectorAll('.level-card');
        if (cards.length >= 3) {
            const nombre = REGION_NAME[continent] || REGION_NAME.unknown;
            const values = ['Banderas de ' + nombre, 'Más países de ' + nombre, 'Resto del mundo'];
            cards.forEach((card, i) => {
                const desc = card.querySelector('.level-desc');
                if (desc && desc.textContent !== values[i]) desc.textContent = values[i];
            });
        }
    }
