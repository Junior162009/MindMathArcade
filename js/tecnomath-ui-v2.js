/* TecnoMath UI V2 — solo mejora presentación. No reemplaza ni controla juegos, patrocinadores o QR. */
(function(){'use strict';
  ...
  function setup(){
    ...
    if(header){
      ...
      if(!$('.tm-ui-controls',header)){
        ...
        controls.innerHTML='<button class="tm-ui-theme" id="tmUiTheme" type="button" aria-label="Cambiar modo de color" aria-pressed="false">☾</button><button class="tm-ui-menu" id="tmUiMenu" type="button" aria-label="Abrir menú" aria-expanded="false">☰</button>';
    ...
    if(hero){
      ...
      if(!$('.tm-ui-hero-actions',hero)){
        ...
        actions.innerHTML='<a class="tm-ui-btn tm-ui-primary" href="#projectsContainer">🎮 EXPLORAR JUEGOS</a><a class="tm-ui-btn tm-ui-secondary" href="#sponsorsArea">🤝 PATROCINADORES</a>';
      ...
      if(!$('.tm-ui-search',hero)){
        ...
        const input=$('#tmUiSearch');
        input.addEventListener('input',()=>filtrarJuegos(input.value));
      }
    }
    ...
    if(menu&&nav&&!menu.dataset.bound){
      ...
      $$('.tm-ui-nav a').forEach(a=>a.addEventListener('click',()=>nav.classList.remove('open')));
    }
    ...
    if(theme&&!theme.dataset.bound){
      ...
      if(saved==='light') applyLight(theme,true);
    ...
    if(!$('#tm-ui-stats')){
      ...
      stats.id='tm-ui-stats'; stats.className='tm-ui-stats'; stats.setAttribute('aria-label','Estadísticas del portal');
      stats.innerHTML='<h3>TECNO<span>MATH</span> EN NÚMEROS</h3><div class="tm-ui-stat-grid"><div class="tm-ui-stat"><strong data-stat="games">0</strong><span>JUEGOS</span></div><div class="tm-ui-stat"><strong data-stat="categories">0</strong><span>CATEGORÍAS</span></div><div class="tm-ui-stat"><strong data-stat="sponsors">0</strong><span>PATROCINADORES</span></div></div>';
      const footer=$('footer');
      const projects=$('#projectsContainer');
      if(projects&&projects.parentNode) projects.parentNode.insertBefore(stats,projects);
      else if(footer) footer.parentNode.insertBefore(stats,footer);
    }
    ...
    if(sponsors) observer.observe(sponsors,{childList:true});
  }

  function applyLight(button,on){
  ...
  function filtrarJuegos(term){
    ...
    const cards=$$('#projectsContainer .project-card');
  ...
  function actualizarEstadisticas(){
    ...
    projects.forEach(card=>{
      ...
      if(desc) categories.add(desc.textContent.trim());
    });
    ...
    Object.entries(values).forEach(([key,target])=>{
      ...
      el.dataset.value=String(target); el.textContent=String(target);
    });
  }
