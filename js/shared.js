...
(function () {
  ...
  const ADMIN_EMAILS=['delahozbarcelojunior@gmail.com']; // Agrega aquí los demás correos.
  ...
  async function ensureFirebaseProfile(user){
    ...
    const data={...current,username:current.username||adminUsername(user),email:user.email,role:'admin',isAdmin:true,provider:user.providerData?.[0]?.providerId||'firebase',updatedAt:firebase.database.ServerValue.TIMESTAMP};
  ...
  // Se ejecuta INMEDIATAMENTE: no esperamos a DOMContentLoaded para crear
  // el perfil admin, evitando la carrera con auth.html.
  function setupAdminBridge(){
    ...
    firebase.auth().onAuthStateChanged(async user=>{
      ...
      if(!user)return;
      if(readyAdmin){
        try{await ensureFirebaseProfile(user)}catch(error){console.error('TecnoMath: error sincronizando admin',error)}
        const button=document.querySelector('#admin-cloud');
        if(button){button.style.display='block';button.innerHTML='👑';button.title='Panel de Control';button.setAttribute('aria-label','Abrir Panel de Control')}
      }
    });
    document.addEventListener('click',event=>{
      const button=event.target.closest?.('#admin-cloud'), user=firebase.auth().currentUser;
      if(!button||!user||(!readyAdmin&&!isAdminEmail(user.email)))return;
      event.preventDefault();event.stopPropagation();event.stopImmediatePropagation();location.assign(dashboardUrl());
    },true);
  }
  setupAdminBridge();
})();
