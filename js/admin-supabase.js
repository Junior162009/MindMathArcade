...
(function () {
  ...
  async function client() {
    ...
    await new Promise((resolve,reject)=>{
      ...
      s.onload=resolve; s.onerror=()=>reject(new Error('No se pudo cargar Supabase.'));
  ...
  async function saveTheme(){try{await rpc('tecnomath_admin_save_setting',{p_key:'theme',p_value:{key:$('themeValue').value}});msg('Temática guardada.');}catch(err){msg(err.message||'No se pudo guardar.','error');}}
  ...
  function bind(){
    ...
    $('refresh').onclick=loadUsers; $('refreshProgress').onclick=loadProgress; $('refreshLogs').onclick=loadLogs; $('loadEvent').onclick=loadSettings; $('refreshThemes').onclick=loadSettings;
    $('eventForm').onsubmit=saveEvent; $('saveUser').onclick=saveUser; $('themeSave').onclick=saveTheme;
    ...
    $('logout').onclick=async()=>{await (await client()).auth.signOut({scope:'local'});location.href='../../pages/auth.html';};
  }
  async function start(){try{bind();const a=await ensureAdmin();$('adminName').textContent=a.profile?.username||a.user.email;await Promise.all([loadUsers(),loadProgress(),loadLogs(),loadSettings()]);msg('Panel Supabase conectado.');}catch(e){console.error(e);msg(e.message||'No autorizado.','error');setTimeout(()=>location.href='../../pages/auth.html',1800);}}
