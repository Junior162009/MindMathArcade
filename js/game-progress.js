...
(function () {
  ...
  function getServices(){if(!window.firebase||!firebase.auth||!firebase.database)return null;return{auth:firebase.auth(),database:firebase.database()}}
  function getUser(){const s=getServices();return s&&s.auth.currentUser}
  function gameKey(id){return String(id||'unknown').replace(/[^a-z0-9_-]/gi,'-').toLowerCase()}
  function gameRef(id){const s=getServices(),u=getUser();if(!s||!u)return null;return s.database.ref('userProgress/'+u.uid+'/games/'+gameKey(id))}
  function totalsRef(){const s=getServices(),u=getUser();if(!s||!u)return null;return s.database.ref('userProgress/'+u.uid+'/totals')}
  ...
  const THEME_DATA={normal:{emoji:'📚',className:'theme-normal',colors:{bg:'#060610',card:'#0d0d1a',border:'#1a1a2e',text:'#ffffff',cyan:'#00FFFF',pink:'#FF00FF',green:'#39FF14',yellow:'#FFE600',gold:'#FFD700'}},halloween:{emoji:'🎃',className:'theme-halloween',colors:{bg:'#10050a',card:'#1b0b16',border:'#5b1f49',text:'#fff4e6',cyan:'#ff7a00',pink:'#ff3d00',green:'#b6ff00',yellow:'#ff9f00',gold:'#ff6a00'}},navidad:{emoji:'🎄',className:'theme-navidad',colors:{bg:'#06120d',card:'#0c2118',border:'#1e5b3b',text:'#ffffff',cyan:'#65e6ff',pink:'#ff3b5c',green:'#39ff7a',yellow:'#ffe66d',gold:'#ffd700'}},verano:{emoji:'☀️',className:'theme-verano',colors:{bg:'#071827',card:'#0c2438',border:'#1f6285',text:'#ffffff',cyan:'#29d9ff',pink:'#ff5ea8',green:'#70ff9a',yellow:'#ffe66d',gold:'#ffd166'}},mundial:{emoji:'🏆',className:'theme-mundial',colors:{bg:'#07100a',card:'#0e1d11',border:'#355b3a',text:'#ffffff',cyan:'#4de1ff',pink:'#e8c547',green:'#4cff70',yellow:'#ffe600',gold:'#ffd700'}},regreso:{emoji:'🎒',className:'theme-regreso',colors:{bg:'#10100a',card:'#1c1c0e',border:'#5d5a24',text:'#ffffff',cyan:'#5fd7ff',pink:'#ff72b6',green:'#75ff75',yellow:'#ffe600',gold:'#ffcc33'}}};
  function applyGlobalTheme(id){id=String(id||'normal').toLowerCase();const theme=THEME_DATA[id]||THEME_DATA.normal,root=document.documentElement,body=document.body;Object.entries(theme.colors).forEach(([k,v])=>{const m={bg:'--theme-bg',card:'--theme-card',border:'--theme-border',text:'--theme-text',cyan:'--neon-cyan',pink:'--neon-pink',green:'--neon-green',yellow:'--neon-yellow',gold:'--gold'};root.style.setProperty(m[k],v)});Object.keys(THEME_DATA).forEach(k=>body&&body.classList.remove(THEME_DATA[k].className));if(body){body.classList.add(theme.className);body.dataset.tecnomathTheme=id}root.dataset.tecnomathTheme=id;root.style.setProperty('--tecnomath-theme',id);root.style.setProperty('--theme-emoji',JSON.stringify(theme.emoji));document.title=`${theme.emoji} TecnoMath · Plataforma de Juegos Educativos`;window.dispatchEvent(new CustomEvent('tecnomath:themechange',{detail:{id,theme}}))}
  function watchGlobalTheme(){const s=getServices();if(!s)return;const ref=s.database.ref('tecnomath/tematicaActiva');ref.on('value',snap=>applyGlobalTheme(snap.val()||'normal'),e=>{console.warn('No se pudo sincronizar la temática global de Tecnomath.',e);applyGlobalTheme('normal')})}
