...
const state={db:null,games:[],counts:new Map(),grades:[],channel:null,ignoredBroadcastIds:new Set(),selectedGame:null};
...
async function loadGrades(){const r=await state.db.from('tecnomath_voting_grades').select('id,grade,group_name,label').eq('active',true).order('grade').order('group_name');if(r.error)throw r.error;state.grades=r.data||[];const select=$('voterGrade');select.innerHTML='<option value="">Selecciona tu grado</option>'+state.grades.map(g=>'<option value="'+esc(g.id)+'">'+esc(g.label)+'</option>').join('')} 
async function refreshView(){const rows=sorted();renderStats(rows);renderGames()}
function openVote(id){const g=state.games.find(x=>gid(x)===id);if(!g)return;state.selectedGame=g;$('voteGameName').textContent=g.name;$('voterName').value='';$('voterGrade').value='';$('voteModal').classList.add('open');$('voterName').focus()}
function closeVote(){$('voteModal').classList.remove('open');state.selectedGame=null}
async function submitVote(){const b=$('confirmVoteBtn'),name=$('voterName').value.trim(),gradeId=$('voterGrade').value;if(!state.selectedGame)return;if(name.length<2){toast('⚠️ Falta el nombre','Escribe el nombre del votante.');$('voterName').focus();return}if(!gradeId){toast('⚠️ Falta el grado','Selecciona tu grado.');return}b.disabled=true;b.textContent='REGISTRANDO…';try{const r=await state.db.functions.invoke('submit-game-vote',{body:{voter_name:name,grade_id:gradeId,game_id:gid(state.selectedGame)}});if(r.error){let msg=r.error.message||'No se pudo registrar el voto.';if(r.data?.error)msg=r.data.error;if(r.status===409||r.data?.duplicate){toast('ℹ️ Voto ya registrado',msg);return}throw Error(msg)}if(!r.data?.ok)throw Error(r.data?.error||'No se pudo registrar el voto.');if(r.data.vote?.id!=null)state.ignoredBroadcastIds.add(String(r.data.vote.id));await loadCounts();await refreshView();closeVote();toast('❤️ Voto registrado','El ranking se actualizará automáticamente.')}catch(e){console.error('TecnoMath voting:',e);toast('⚠️ No se pudo registrar',e.message||'Inténtalo nuevamente.')}finally{b.disabled=false;b.textContent='REGISTRAR VOTO'}}
