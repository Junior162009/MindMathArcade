...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const student = esc(data.student);
    ...
    const html = `<!doctype html><html lang="es"><body style="font-family:Arial,sans-serif;line-height:1.5;color:#172033">
      <div style="max-width:680px;margin:auto;padding:24px">
        ...
        <p><b>Correctas:</b> ${correct} · <b>Respondidas:</b> ${answered}</p>
        <p><b>Tiempo:</b> ${time}</p>
    ...
    const resend = await fetch('https://api.resend.com/emails', {
      ...
      body: JSON.stringify({
        from: FROM_EMAIL,
        to: [TEACHER_EMAIL],
        subject: `📝 Quiz Reseña Crítica — ${String(data.student || 'Estudiante')} — ${score}/${total}`,
        html,
      }),
    });

    const result = await resend.text();
    if (!resend.ok) throw new Error(`Resend ${resend.status}: ${result}`);
