...
function slugify(value){
  return String(value||'juego').normalize('NFD').replace(/[\u0300-\u036f]/g,'').toLowerCase().replace(/[^a-z0-9]+/g,'-').replace(/^-+|-+$/g,'').slice(0,48)||'juego';
}
function sleep(ms){return new Promise(r=>setTimeout(r,ms));}
async function github(path,options={}){
  const r=await fetch(`https://api.github.com${path}`,{...options,headers:{accept:'application/vnd.github+json',authorization:`Bearer ${GITHUB_TOKEN.value()}`,...(options.headers||{})}});
...
exports.notifyGameSubmission=onValueCreated({ref:'/gameSubmissions/{submissionId}',secrets:[RESEND_API_KEY,EMAIL_FROM]},async event=>{
  const data=event.data.val();if(!data)return;
  const key=RESEND_API_KEY.value();if(!key)return;
...
exports.publishApprovedGame=onValueWritten({ref:'/gameSubmissions/{submissionId}',secrets:[GITHUB_TOKEN]},async event=>{
  ...
  const files=[];let commonRoot=null;
  for(const [name,entry] of Object.entries(zip.files)){
    ...
    if(content.length>5*1024*1024)throw new Error(`Archivo demasiado grande: ${clean}`);
    files.push({name:clean,content});
  }
  if(!files.some(f=>f.name.toLowerCase()==='index.html')){
    const index=files.find(f=>f.name.toLowerCase().endsWith('/index.html'));
    if(index)commonRoot=index.name.slice(0,index.name.toLowerCase().lastIndexOf('/index.html'))+'/' ;
  }
  ...
  const head=await github(`/repos/${OWNER}/${REPO}/git/ref/heads/${BRANCH}`);
  const headSha=head.object.sha;
  const commit=await github(`/repos/${OWNER}/${REPO}/git/commits/${headSha}`);
  const entries=[];
  for(const file of files){
    const blob=await github(`/repos/${OWNER}/${REPO}/git/blobs`,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({encoding:'base64',content:file.content.toString('base64')})});
    entries.push({path:`${folder}/${file.name}`,mode:'100644',type:'blob',sha:blob.sha});
  }
