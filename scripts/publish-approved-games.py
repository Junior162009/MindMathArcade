...
headers = {'User-Agent': 'TecnoMath-GitHub-Publisher/8.0', 'Accept': 'application/json', 'Authorization': f'Bearer {ACCESS_TOKEN}'}
...
def firebase_patch(path, payload):
    ...
    req = urllib.request.Request(f'{DB}/{path}.json', data=body, headers={**headers, 'Content-Type': 'application/json'}, method='PATCH')
...
games_dir = Path('games'); data_dir = Path('data')
games_dir.mkdir(parents=True, exist_ok=True); data_dir.mkdir(parents=True, exist_ok=True)
...
for submission_id, game in queue.items():
    ...
    try:
        if source_type == 'url':
            ...
            page = f'<!doctype html><html lang="es"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>{title}</title></head><body><script>location.replace({json.dumps(game_url)});</script><noscript><a href={json.dumps(game_url)}>Abrir juego</a></noscript></body></html>'
            (destination / 'index.html').write_text(page, encoding='utf-8')
        else:
            encoded = game.get('packageBase64')
            if not encoded: raise RuntimeError('El envío aprobado no contiene el paquete del juego.')
            work = Path('/tmp') / f'tecnomath-{submission_id}'; shutil.rmtree(work, ignore_errors=True); work.mkdir(parents=True, exist_ok=True)
            install_game(unpack(encoded, work), destination)
        entry = {'name': name, 'desc': str(game.get('description') or 'Juego educativo de TecnoMath'), 'url': f'games/{folder}/index.html', 'imageUrl': str(game.get('imageUrl') or ''), 'icon': str(game.get('icon') or '🎮'), 'category': str(game.get('category') or 'otros'), 'deviceCompatibility': 'both', 'evento': None, 'submissionId': submission_id, 'authorName': str(game.get('authorName') or 'Usuario'), 'authorEmail': str(game.get('authorEmail') or ''), 'sourceType': source_type, 'publishedAt': datetime.now(timezone.utc).isoformat()}
        catalog.append(entry); published_ids.add(submission_id); changed = True; published.append((submission_id, game, entry))
...
if changed:
    ...
    if subprocess.run(['git','diff','--cached','--quiet']).returncode != 0:
        subprocess.run(['git','commit','-m','feat: publish approved games and notify authors'], check=True); subprocess.run(['git','push'], check=True)
    for submission_id, game, entry in published:
        firebase_patch(f'gameSubmissions/{submission_id}', {'status':'published','publishedAt':Date.now() if False else datetime.now(timezone.utc).isoformat(),'publishedUrl':f'{SITE_URL}/{entry["url"]}'})
        firebase_patch(f'gameUploadQueue/{submission_id}', {'status':'published','publishedAt':datetime.now(timezone.utc).isoformat(),'publishedUrl':f'{SITE_URL}/{entry["url"]}'})
        email = str(game.get('authorEmail') or '').strip()
        if email:
            game_name = html.escape(entry['name']); game_url = f'{SITE_URL}/{entry["url"]}'
            body = f'<div style="font-family:Arial,sans-serif"><h2>🎉 ¡Tu juego ya está publicado!</h2><p>Hola {html.escape(str(game.get("authorName") or ""))},</p><p>Tu juego <b>{game_name}</b> fue aprobado y ya está disponible en TecnoMath.</p><p><a href="{html.escape(game_url)}">🎮 Abrir mi juego</a></p><p>¡Gracias por crear con TecnoMath!</p></div>'
